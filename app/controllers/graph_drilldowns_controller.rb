class GraphDrilldownsController < ApplicationController
  ...
  def rev_by_year
    ...
    referral_by_year_high = Graph.fee_estimate_by_year(current_user,"estimated_conclusion_#{@recovery_rate}","high_referral").map { |x| x*-1}
    @referral_by_year = Graph.fee_estimate_by_year(current_user,"estimated_conclusion_#{@recovery_rate}","medium_referral").map { |x| x*-1}
    referral_by_year_low = Graph.fee_estimate_by_year(current_user,"estimated_conclusion_#{@recovery_rate}","low_referral").map { |x| x*-1}
